    ...
    <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <form action="{{ route('admin.links.update', $link->id) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $link->name) }}" required maxlength="100" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4318FF]/20 focus:border-[#4318FF] transition-all" placeholder="e.g. WhatsApp, Facebook, Book Now">
                @error('name')
            ...
            <div class="mb-5">
                ...
                <input type="url" name="link" id="link" value="{{ old('link', $link->link) }}" required maxlength="500" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4318FF]/20 focus:border-[#4318FF] transition-all" placeholder="https://wa.me/977XXXXXXXXXX">
                @error('link')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-5">
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $link->slug) }}" maxlength="100" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4318FF]/20 focus:border-[#4318FF] transition-all" placeholder="Auto-generated from name">
            ...
            <div class="mb-6">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $link->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-[#4318FF] focus:ring-[#4318FF]">
